...
import static io.quarkiverse.openapi.problem.deployment.ExceptionMapperDefinition.mapper;
...
public class ProblemProcessor {
    ...
    private static final String FEATURE_NAME = "openapi-problem";
    private static final String EXTENSION_MAIN_PACKAGE = "io.quarkiverse.openapi.problem.";
    ...
    private static List<ExceptionMapperDefinition> neededExceptionMappers() {
        Stream<ExceptionMapperDefinition> allMappers = Stream.of(
                mapper(EXTENSION_MAIN_PACKAGE + "HttpProblemMapper")
                        .thatHandles("io.quarkiverse.openapi.problem.HttpProblem"),
    ...
    FeatureBuildItem createFeature(Capabilities capabilities) {
        if (RESTEASY_JSON_CAPABILITIES.stream().noneMatch(capabilities::isPresent)) {
            logger().error("`quarkus-openapi-problem` extension is useless without a REST Json Provider. Please add "
                    + "`quarkus-rest-jackson` or `quarkus-rest-jsonb` (or reactive versions) to your pom.xml.");
        }
